...
// Proje ayarları
const projectSetup = {
	projectName: "openpayd2025", // WordPress tema adı veya HTML projesi adı
			...
			output: {
				...
				assetFileNames: ({ name }) => {
					...
					if (/\.(webp|png|jpg|jpeg|gif|ico|svg)$/.test(name ?? "")) {
						return `${removeLeadingSlash(assetsPath)}/images/[name][extname]`;
					}
					if (/\.(woff|woff2|ttf)$/.test(name ?? "")) {
						return `${removeLeadingSlash(assetsPath)}/fonts/[name][extname]`;
					}
					if (/\.(mp4|webm|ogg|mov)$/.test(name ?? "")) {
						return `${removeLeadingSlash(assetsPath)}/videos/[name][extname]`;
					}
					return "/[name][extname]";
				},
				// manualChunks: () => "app",
